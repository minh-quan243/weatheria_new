...
# =========================
# 4. Tổng hợp theo tháng và tạo cột "có bão hay không"
# =========================
...
# Tạo cột "is_typhoon" (1 nếu có bão, 0 nếu không có bão)
merged["is_typhoon"] = merged["num_typhoons"].apply(lambda x: 1 if x > 0 else 0)

# Giữ lại chỉ các cột cần thiết: năm, tháng, mùa, is_typhoon
final_df = merged[["year", "month", "season", "is_typhoon"]]
...
final_df.to_csv(f"{output_path}/istyphoon_dataset.csv", index=False)
print("✅ Dataset đã lưu tại:", f"{output_path}/istyphoon_dataset.csv")
